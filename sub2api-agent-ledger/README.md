...
- **代理商用自己的余额创建卡密**：代理商自助端只有卡密核销（`POST /api/agent/cards/redeem`），没有铸卡入口。目前只有管理员能创建卡密（`POST /api/admin/cards/batches`，从指定代理商钱包扣减）。补齐需要新增服务端路由：校验余额充足 → 扣减钱包 → 生成卡密，三步同事务。
- **管理员解绑用户 / 作废卡密**：服务端已有 `POST /api/admin/assignments/:id/unbind` 和 `POST /api/admin/cards/:id/revoke`，前端未接入。
