...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not link payment guidance to external GitHub docs", async () => {
    const wrapper = mountView();

    await flushPromises();
    await openPaymentTab(wrapper);

    const githubDocLinks = wrapper
      .findAll("a")
      .filter((node) => {
        const href = node.attributes("href") || "";
        return href.includes("github.com") && href.includes("docs/PAYMENT");
      });

    expect(githubDocLinks).toHaveLength(0);
